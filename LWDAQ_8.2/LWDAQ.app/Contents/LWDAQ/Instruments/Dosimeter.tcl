...
proc LWDAQ_analysis_Dosimeter {{image_name ""}} {
	...
	# If the threshold is to be a function of the image intensity, we
	# we call lwdaq_bcam.
	set threshold $config(analysis_threshold)
	if {![string is integer -strict $threshold]} {
		set threshold_string [lwdaq_bcam $image_name \
			-num_spots 1 \
			-threshold $config(analysis_threshold) \
			-analysis_type 1]
		if {[LWDAQ_is_error_result $threshold_string]} {return $threshold_string}
		set threshold [lindex $threshold_string 5]
	}

	# To obtain the total image charge above threshold, we use a histogram of
	# image intensity.
	...
	# boundaries to obtain the charge density in counts/pixel. Obtain
	# the average intensity of the image without threshold subtraction.
	...
	set hits [lwdaq_bcam $image_name \
		...
		-return_intensity 1 \
		-sort_code 1]
	if {[LWDAQ_is_error_result $hits]} {
		set hits ""
		for {set i 1} {$i <= $config(analysis_num_spots)} {incr i} {append hits "-1 "}
	}
