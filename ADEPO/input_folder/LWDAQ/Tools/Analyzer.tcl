...
proc Analyzer_init {} {
	...
	set config(max_current_error) 20
...
proc Analyzer_analyze {ip_addr base_addr driver_socket mux_socket} {
	...
	lwdaq_image_create -name $info(data_image) \
		...
		-height [expr ([llength $config(commands)] + 3) * $info(lines_per_measurement) \
			+ 1 ]
	...
	foreach {d s} $info(signatures) {
		...
		set value [format %.1f [expr sqrt($value)]]
		lappend values [list $d $value $loopback $termination]
	}
	...
	set values [lsort -increasing -real -index 1 $values]
	set device [lindex $values 0 0]
	set current_error [lindex $values 0 1]
	set loopback [lindex $values 0 2]
	set termination [lindex $values 0 3]
	...
	Analyzer_print "Current Error = $current_error mA."	
	set problem ""
	if {$current_error > $config(max_current_error)} {
	...
	set result "$ip_addr $base_addr $driver_socket $mux_socket\
		$mux_repeater $excessive $current_error \"$result\""
...
termination yes
0  0 0 0.0 0  0 increase_with_0000
0 70 0 0.3 0 65 increase_with_0080
0  0 0 0.0 0  0 increase_with_0001
0 37 0 0.3 0 32 increase_with_1080
0 70 0 0.3 0 65 increase_with_0480
0 85 0 0.3 0 65 increase_with_8280
