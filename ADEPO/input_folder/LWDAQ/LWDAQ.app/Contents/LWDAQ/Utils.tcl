...
proc LWDAQ_socket_write {sock data} {
	...
	if {[catch {
		...
		LWDAQ_socket_close $sock
		error "Error writing to $sock, $error_result."
	}
	return 1
}
...
proc LWDAQ_get_file_name {{multiple 0}} {
	...
	set fl [tk_getOpenFile \
		-initialdir $LWDAQ_Info(working_dir) \
		-multiple $multiple]
	if {$fl == ""} {return ""}
	if {$multiple} {
...
proc LWDAQ_get_dir_name {} {
	...
	set dirname [tk_chooseDirectory \
		-initialdir $LWDAQ_Info(working_dir) \
		-mustexist 0]
	if {$dirname != ""} {set LWDAQ_Info(working_dir) $dirname}
	return $dirname
}
