...
type
	...
	spot_list_type(length:integer)=record
		...
		current_stack_depth:integer;{to avoid stack overflow}
...
var
	i,j,sum,max,min,sum_i,sum_j,step_sum,step_sum_i,step_sum_j:integer;
	count,net_intensity:integer;
...

{
	...
	which means our recursive calling of this routine will run the
	stack pointer up by a megabyte or more. We protect against memory
	access violation by limiting the depth of our do_pixel recursion
	with the max_stack_depth constant. When we reach the maximum stack
	depth, do_pixel will not call itself, but instead returns. When
	the stack counter drops below the maximum, do_pixel can call
	itself and add more pixels to the spot. With max_stack_depth set
	to 1000, we cannot add pixels that are more than 1000 steps away
	from the initial pixel. We find that the entire spot is almost
	always joined together by taking the combination of all
	non-overlapping paths from the initial pixel that are less than
	1000 pixels long.
}
		...
		if current_stack_depth<max_stack_depth then begin
			...
			with ip^,ip^.analysis_bounds,spots[num_valid_spots] do begin
				if (intensity[j,i]>threshold) and (overlay[j,i]=clear_color) then begin
					...
					if kitty_corners then begin
						if j<bottom then begin
							if i>left then do_pixel(slp,j+1,i-1);
							if i<right then do_pixel(slp,j+1,i+1);
						end;
						if j>top then begin 
							if i>left then do_pixel(slp,j-1,i-1);
							if i<right then do_pixel(slp,j-1,i+1);
