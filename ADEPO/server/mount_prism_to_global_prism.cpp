...
void mount_prism_to_global_prism(BDD & base_donnees, Configuration &config, bool airpads)
{
    bool found = false;
    for(unsigned int i=0; i<base_donnees.getMountCoordPrisms().size(); i++)
    {
        MountCoordPrism prism = base_donnees.getMountCoordPrisms().at(i);
        float airpad = airpads ? config.getDetector(prism.getBCAM().getName()).getAirpad() : 0.0f;

        for(unsigned int j=0; j<base_donnees.getBCAMParams().size(); j++)
        {
            BCAMParams params = base_donnees.getBCAMParams().at(j);
