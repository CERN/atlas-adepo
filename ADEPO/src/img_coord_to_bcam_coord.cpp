...
{
    bool found = false;
    std::cout << "Looking for: " << base_donnees.Get_liste_calib1().at(j).Get_id_BCAM() << std::endl;
    for(unsigned int i=0; i<base_donnees.Get_liste_spots().size(); i++) //je parcours la base de donnees des coordonnees images
    {
        ...
        {
            std::cout << "Checking: " << base_donnees.Get_liste_spots().at(i).Get_nom_BCAM_Objet().substr(0,14) << std::endl;
