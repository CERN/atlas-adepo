...
public:
	...
	* @param data - OWNERSHIP remains with caller
	*/
	void write(const void * data, const unsigned int noBytes) throw (std::runtime_error);
	...
	*/
	void write(const void * data, const unsigned int noBytes, const unsigned int startOffset_byte)  throw (std::runtime_error);

	/**
	* Read noBytes from current position placing results in data
	* @returns ptr to data - not owned
	*/
	const void * read(const unsigned int noBytes)  throw (std::runtime_error);
	...
	* @returns ptr to data - not owned
	*/
	const void * read(const unsigned int noBytes, const unsigned int startOffset_byte)  throw (std::runtime_error);
	...
	*/
	const char * readString() throw (std::runtime_error);

	/**
	* Read String (null terminated) starting from startOffset_byte
	* @returns ptr to string - not owned
	*/
	const char * readString(const unsigned int startOffset_byte)  throw (std::runtime_error);

	/*
	* @returns ptr to string - not owned
	*/
	static const char * readString(const void * data, const unsigned int dataByteSize, const unsigned int startOffset_byte)  throw (std::runtime_error);
};
