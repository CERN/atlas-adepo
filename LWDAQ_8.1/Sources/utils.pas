...
function length_x_graph(gp:x_graph_ptr_type):real;
...
function length_xy_graph(gp:xy_graph_ptr_type):real;
...

{
	...
	starting at a^. The code supports either 32-bit or 64-bit
	word sizes, based upon the size of the cardinal type.
}
procedure block_set(a:pointer;length:integer;value:byte);
var c:cardinal;
begin
	...
	c:= c or (c shl 8);
	c:= c or (c shl 16);
	if sizeof(cardinal) = 8 then c:= c or (c shl 32);
...
{
	bubble_sort arranges the elements of a list in ascending order, as
	defined by the "greater" function, and does so by applying the "swap" 
	function.
	...
	n as parameters. If the m'th element in the list should come after the n'th
	element, greater(m,n) should return true. Otherwise greater(m,n) should
	return false. By means of swap(m,n) and greater(m,n), the bubble_sort routine,
...
{
	Sort the simplex vertices into order of ascending altitude. We use
...
procedure simplex_sort(var simplex:simplex_type);
	...
	begin 
		with simplex do begin
			greater:=(altitudes[i]>altitudes[j]);
		end;
	end;
...
begin
	quick_sort(1,simplex.n+1,swap,greater);
end;
		...
		a_reflect:=altitude(v_reflect);
{
	...
	n vertices, we keep it in place of the worste vertex.
		...
		if (a_reflect>=altitudes[1]) and (a_reflect<altitudes[n]) then begin
			...
			if report then writeln('r: ',a_reflect);
		end;
{
	...
	the highest vertex.
		...
		if (a_reflect<altitudes[1]) then begin
			...
			if a_expand<a_reflect then begin
				...
				if report then writeln('re: ',a_expand);
			end else begin
				vertices[n+1]:=v_reflect;
				altitudes[n+1]:=a_reflect;
				if report then writeln('rl: ',a_reflect);
			end;
		end;
{
		...
		if (a_reflect>=altitudes[n]) then begin
			...
			if a_contract<=altitudes[n+1] then begin
				...
				if (simplex_size(simplex)<construct_size*small_size_factor) then 
					inc(done_counter);
				if report then writeln('c: ',a_contract);
			end else begin
{
				for i:=2 to n+1 do begin
				...
				if report then writeln('s: ',a_contract);
}
				inc(done_counter);
				if done_counter<max_done_counter then begin
					simplex_construct(simplex,altitude);
					if report then writeln('x: ',a_contract);
				end else if report then writeln('d: ',a_contract);
			end;
		end;
	end;
{
	...
	simplex_sort(simplex);
{
	When reporting, we print out the entire list of vertices with their altitudes.
}
	if report then with simplex do begin
