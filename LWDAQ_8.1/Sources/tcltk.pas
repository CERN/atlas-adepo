...
type
	...
	Tcl_CmdProc=
		function(data,interp:pointer;argc:integer;var argv:Tcl_ArgList):cinteger;
	Tcl_CmdDeleteProc=procedure(data:pointer);
	Tk_PhotoPixelElement=(red,green,blue,alpha);
	Tk_PhotoImageBlock=record
		pixelptr:pointer;
		width,height,pitch,pixelSize:cinteger;
		offset:array [Tk_PhotoPixelElement] of cinteger;
	end;
...
function Tcl_CreateObjCommand (interp:pointer;s:CString;cmd:Tcl_CmdProc;
	data:cinteger;delete_proc:Tcl_CmdDeleteProc):pointer;
	external name 'Tcl_CreateObjCommand';
function Tcl_EvalFile (interp:pointer;s:CString):cinteger;
	external name 'Tcl_EvalFile';
function Tcl_GetByteArrayFromObj(obj_ptr:pointer;var size:cinteger):pointer;
	external name 'Tcl_GetByteArrayFromObj';
function Tcl_GetObjResult (interp:pointer):pointer;
	external name 'Tcl_GetObjResult';
function Tcl_GetStringFromObj (obj_ptr:pointer;var size:cinteger):CString;
	external name 'Tcl_GetStringFromObj';
function Tcl_GetVar(interp:pointer;name:CString;flags:cinteger):CString;
	external name 'Tcl_GetVar';
function Tcl_Eval (interp:pointer;s:CString):cinteger;
	external name 'Tcl_Eval';
function Tcl_InitStubs (interp:pointer;s:CString;e:cinteger):pointer;
	external name 'Tcl_InitStubs';
function Tcl_NewByteArrayObj(bp:pointer;size:cinteger):pointer;
	external name 'Tcl_NewByteArrayObj';
function Tcl_PkgProvide (interp:pointer;name:CString;version:CString):cinteger;
	external name 'Tcl_PkgProvide';
procedure Tcl_SetByteArrayObj(obj_ptr,bp:pointer;size:cinteger);
	external name 'Tcl_SetByteArrayObj';
procedure Tcl_SetObjResult (interp,obj_ptr:pointer);
	external name 'Tcl_SetObjResult';
procedure Tcl_SetStringObj (obj_ptr:pointer;s:CString;l:cinteger);
	external name 'Tcl_SetStringObj';
function Tcl_SetVar(interp:pointer;name:CString;value:CString;flags:cinteger):CString;
	external name 'Tcl_SetVar';
function Tk_FindPhoto(interp:pointer;imageName:CString):pointer;
	external name 'Tk_FindPhoto';
function Tk_InitStubs (interp:pointer;s:CString;e:cinteger):pointer;
	external name 'Tk_InitStubs';
procedure Tk_PhotoBlank(handle:pointer);
	external name 'Tk_PhotoBlank';
procedure Tk_PhotoSetSize(inerp:pointer;handle:pointer;width,height:cinteger);
	external name 'Tk_PhotoSetSize';
function Tk_PhotoGetImage(handle,blockptr:pointer):cinteger;
	external name 'Tk_PhotoGetImage';
procedure Tk_PhotoPutBlock(interp,handle,blockptr:pointer;x,y,width,height,comprule:cinteger);
	external name 'Tk_PhotoPutBlock';
procedure Tk_PhotoPutZoomedBlock(interp,handle,blockptr:pointer;x,y,width,height,
		zoomX,zoomY,subsampleX,subsampleY,comprule:cinteger);
	external name 'Tk_PhotoPutZoomedBlock';
{
	Tcl routines implemented as macros in the C header file, here re-constructed
	in pascal.
}
function Tcl_IsShared(obj_ptr:pointer):cinteger;
...
function Tcl_ObjInteger(obj_ptr:pointer):cinteger;
...
function Tcl_RefCount(obj_ptr:pointer):cinteger;
...
procedure Tcl_SetReturnByteArray(interp,bp:pointer;size:cinteger);
...
function Tcl_ObjShortString(obj_ptr:pointer):short_string;
var s:short_string;size:cinteger;
begin
	s:=short_string_from_c_string(Tcl_GetStringFromObj(obj_ptr,size));
	Tcl_ObjShortString:=s;
end;
...
function Tcl_ObjLongString(obj_ptr:pointer):long_string_ptr;
var fp:long_string_ptr;size:cinteger;
begin
